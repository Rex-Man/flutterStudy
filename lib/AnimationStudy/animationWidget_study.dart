...
///
class ScaleAnimationWidgetStudy extends StatefulWidget {
  @override
  _ScaleAnimationRouteState createState() => new _ScaleAnimationRouteState();
}

//需要继承TickerProvider，如果有多个AnimationController，则应该使用TickerProviderStateMixin。
class _ScaleAnimationRouteState extends State<ScaleAnimationWidgetStudy>
  ...
  Widget build(BuildContext context) {
    print("call Father in animation widget");
...
class AnimatedImage extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    final Animation<double> animation =
        listenable; // 不用显式的去添加帧监听器，然后再调用setState() 了，这个好处和AnimatedWidget是一样的。
    return Scaffold(
      appBar: AppBar(
        title: Text("缩放简单动画"),
      ),
